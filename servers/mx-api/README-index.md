...
>[!NOTE]
>Using the Index server has the advantage of having all the tools always active in the current context, and the disadvantage of increased token usage when communicating with AI models (depending on the implementation of the AI agent and MCP client, each request can includes all tool definitions and parameters).
>Use it for cases when you don't know what specific tools will be needed, or when access to all tools is always required.
>For other cases, use the specialized servers, to have only the necessary tools active in the context.
